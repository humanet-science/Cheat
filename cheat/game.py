...
class CheatGame:
    def __init__(
        ...
        *,
        ...
        self._replacing: set = (
            set()
        )  # player IDs currently being replaced, guards against concurrent calls

        # Create a folder for the game results
        _date_time = datetime.now().strftime("%Y%m%d_%H%M%S")
        if out_dir is not None:
            out_path = os.path.expanduser(os.path.join(out_dir, f"game_{_date_time}"))
            if note:
                out_path += f"_{note}"
    ...
    def call_bluff(self, caller_idx):
        ...
        self.log(
            GameAction(
                type="call",
                player_id=caller_idx,
                timestamp=datetime.now(),
                data=dict(
                    was_lying=lying,
                    accused_id=last_player,
                    revealed_cards=[str(c) for c in cards_played],
                ),
            )
        )
        ...
        if lying:
            ...
            self.log(
                GameAction(
                    type="pick_up",
                    player_id=self.players[last_player].id,
                    timestamp=datetime.now(),
    ...
    def get_info(self) -> dict:
        ...
        return {
            ...
            "pile_size": len(self.pile),
            "human_ids": [
                player.id for player in self.players if player.type == "human"
            ],
    ...
    async def send_state_to_all(self):
        ...
        for player in self.players:
            await player.send_message(
                {"type": "state", **player.get_info(), **self.get_info()}
            )
    ...
    async def handle_message(self, player, data):
        ...
        if data.get("type") == "new_round":
            ...
            await self.broadcast_to_all(
                {"type": "player_ready", "player_id": player.id}
            )
    ...
    async def play_round(self, *, sleep_pause: float = 1.0):
        ...
        while not self.round_over:
            ...
            # Back off while _delayed_replace is pending — avoids a 2 Hz spin in the logs
            if current_player.type == "human" and not current_player.connected:
            ...
            if current_player.type == "human":
                ...
                while data is None:
                    try:
                        ...
                        if not turn_acknowledged and getattr(
                            current_player, "turn_acknowledged", False
                        ):
                            current_player.turn_acknowledged = False
                            idle_ticks = 0
                        ...
                        if not current_player.connected:
                            break  # player already disconnected naturally; _delayed_replace handles the rest
                        ...
                        if self.timeout is not None and turn_acknowledged:
                            ...
                            if idle_ticks % 2 == 0 and elapsed == self.timeout // 3:
                                ...
                                try:
                                    ...
                                    pass
                                break  # one-shot: stop waiting, let _delayed_replace handle cleanup

                        continue  # Still human, keep waiting

                # If player was replaced or timed out, skip to next iteration
                if current_player.type != "human" or data is None:
                    continue
    ...
    async def replace_player_with_bot(self, player):
        ...
        if player.id in self._replacing or self.players[player.id] is not player:
            return
        self._replacing.add(player.id)
    ...
    async def wait_for_new_round(self):
        ...
        while True:
                ...
                await self.broadcast_to_all(
                    {
                        ...
                        "waiting_for_players": [
                            ...
                            and player.id not in self._new_round_confirmations
                        ],
                    }
                )
            ...
            if elapsed > timeout:
                ...
                if len(self._new_round_confirmations) == 0:
                    ...
                    self.logger.info(
                        f"{len(self._new_round_confirmations)} player(s) confirmed, continuing"
                    )
                    for player in self.players:
        ...
        self.logger.info(
            f"Player id(s) {', '.join([str(id) for id in self._new_round_confirmations])} confirmed, starting new round"
        )
