...
from cheat.card import Card, str_to_Card, WORD_TO_RANK, WORD_TO_SUIT, RANKS, SUITS
...
def generate_client_input(kind, *, system_prompt, game_summary, additional_prompts: str = None) -> dict:
    if kind == 'deepseek':
        ...
        if additional_prompts:
            res = dict(config=types.GenerateContentConfig(
                system_instruction=system_prompt + f" Remember: {additional_prompts}."), contents=game_summary)
        else:
            res = dict(config=types.GenerateContentConfig(system_instruction=system_prompt), contents=game_summary)
        return res
...
def convert_LLM_response(game, response: str, *, player_id: int = None) -> GameAction:
    ...
    # Check for a play
        ...
        return GameAction(type='play', player_id=player_id, data=dict(declared_rank=declare_rank, cards_played=cards_to_play))
...
def is_valid_move(move: GameAction, game, hand: list[Card]) -> tuple[bool, str]:
    ...
    if move.type != 'call' and move.data is None:
        return False, "When playing, you must select up to 3 cards to play from your hand."
    if move.type != 'play' and game.current_rank is None:
        return False, "You must select between one and three cards to play from your hand and declare a rank."
    if move.type == 'play':
        ...
        if move.data.get('declared_rank') is not None and game.current_rank is not None and move.data.get('declared_rank') != game.current_rank:
            return False, f"You cannot declare a different rank from the one current being played; current rank is: {game.current_rank}"
        ...
        if len(move.data.get('cards_played', [])) == 0 or len(move.data.get('cards_played', [])) > 3:
            return False, "You must select between one and three cards to play from your hand."
...
class LLM_Player(Player):
    ...
    def move_from_LLM_response(self, game, *, additional_prompts: str = "") -> GameAction:
        ...
        game.log(GameAction(type="LLM_response", player_id=self.id, timestamp=datetime.now(), data=dict(response=response)))
    ...
    def disconnect(self) -> GameAction:
        ...
        return GameAction(type="failure", player_id=self.id, timestamp=datetime.now(), data=dict(reason="invalid response"))
    ...
    async def make_move(self, game):
        ...
        if not is_valid:
            move = self.move_from_LLM_response(game, additional_prompts=additional_prompts)
            is_valid, additional_prompts = self.check_LLM_move_valid(game, move)
