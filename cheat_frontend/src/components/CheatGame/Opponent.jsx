	...
	return (opponents.map((opp, index) => {
		...
		>
			<div className="text-center flex flex-col items-center justify-center h-full relative">
				...
				{/* Avatar in center */}
				<span
					className="text-2xl md:text-4xl lg:text-5xl z-10 relative group-hover:scale-110 transition-all">{opp.avatar}</span>
			</div>
  ...
  onMouseLeave={() => setHoveredPlayer(null)}
			>
				{playAnnouncements.filter(msg => msg.playerId === opp.id).map((msg, i, arr) => {
					...
					return (<div
						key={`small-${msg.id}`}
						className={`absolute backdrop-blur-lg drop-shadow-xl
						rounded-2xl p-2 bg-amber-200 ${i !== arr.length - 1 ? 'bg-opacity-low' : 'bg-opacity-high'} text-xs play-announcement-small transition-all duration-300`}
						style={{
							bottom: '50%', left: '90%', // '--tw-translate-y': `${-i * 30}%`,
							// '--tw-translate-x': `${i * 5}%`,
							zIndex: 40+i,
							transform: `translate(${isHovered ? offsetX : 10}px, ${isHovered ? offset : -10*i}px)`,
						}}
					>
