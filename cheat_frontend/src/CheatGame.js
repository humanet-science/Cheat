...
export default function CheatGame() {
  ...
  const processActionQueue = async () => {
    	...
    	setAnimatingCards(null);
			...
			setPileCards(prev => [
					...prev,
					...Array(msg.card_count)
						.fill(0)
						.map(() => ({
							id: Math.random(),
							rotation: Math.random() * 60 - 10, // random rotation
	...
	useEffect(() => {
		...
		socket.onmessage = async (event) => {
		  ...
		  if (["state", "state_update", "card_played", "bluff_called"].includes(msg.type)) {
	...
	<div className="min-h-screen text-white flex flex-col p-6">
			...
			<div className="flex-1 flex items-center justify-center relative">
				...
				{/* Pile in center */}
				<div className="fixed left-1/2 top-1/2 z-30" style={{ transform: "translate(-50%, -50%)"}}>
				...
				{revealedCards && (
					<div className="absolute inset-0 bg-black bg-opacity-70 flex items-center justify-center z-40">
						<div className="bg-blue-800 rounded-xl p-8 border-4 border-yellow-400">
							<h3 className="text-2xl font-bold text-center mb-4">
								{revealedCards.wasLying ? "🎭 Caught Lying!" : "✓ Telling Truth!"}
							</h3>
							<div className="text-center mb-4">
								<p>Player {revealedCards.accused} declared: <span className="text-yellow-400 font-bold">{revealedCards.declaredRank}</span></p>
								<p className="text-sm opacity-75">Actually played:</p>
							</div>
							<div className="flex justify-center gap-2">
								{revealedCards.cards.map((card, i) => {
									...
									return (
										<div
											key={i}
											className="relative w-16 h-24 rounded-lg border-2 bg-white border-gray-400 shadow-lg"
											style={{
												animation: `flipCard 0.6s ease-out ${i * 0.2}s both`
											}}
										>
			...
			{animatingCards && (
				...
				>
					<div className="flex gap-1">
						{Array(animatingCards.cardCount).fill(0).map((_, i) => (
							<div
								key={i}
								className="w-12 h-16 bg-blue-600 rounded border-2 border-white shadow-lg"
								style={{
